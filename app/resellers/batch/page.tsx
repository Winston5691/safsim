...
const MOCK_BATCH_RESULTS = [
  { id: "RS-1001", status: "Valid", message: "Compliant" },
  { id: "RS-1002", status: "Valid", message: "Compliant" },
  { id: "RS-1003", status: "Invalid", message: "Reseller ID not found" },
  { id: "RS-1004", status: "Valid", message: "Compliant" },
  { id: "RS-1005", status: "Duplicate", message: "Reseller ID already listed" },
];

export default function ResellersBatchPage() {
    ...
    <Container maxWidth={false} className="animate-fadeInUp" sx={{ p: 0 }}>
      <Stack spacing={2} sx={{ pt: 1, pb: 4 }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 0.5, px: { xs: 2, md: 3 } }}>
           <Typography variant="h5" fontWeight={600}>Batch Reseller Validation</Typography>
           <Button variant="outlined" component={NextLink} href="/resellers" startIcon={<ArrowBack />} size="small" sx={{ fontWeight: 600, borderRadius: 1.5, textTransform: "none" }}>Back to List</Button>
        </Box>
           ...
           <Box sx={{ p: 3 }}>
              ...
              {activeStep === 0 && (
                <Box sx={{ maxWidth: 600, mx: "auto" }}>
                  <FormControl>
                    <FormLabel sx={{ fontSize: "0.85rem", mb: 1 }}>How would you like to submit IDs?</FormLabel>
                    <RadioGroup
                      value={batchMethod}
                    ...
                    >
                      <FormControlLabel
                        ...
                        label={<Box><Typography variant="body2" fontWeight={600}>Upload CSV File</Typography><Typography variant="caption" color="text.secondary">Upload a CSV file with internal IDs</Typography></Box>}
                        sx={{ mr: 4, alignItems: "flex-start", mt: 0.5 }}
                      />
                      <FormControlLabel
                        value="range"
                        control={<Radio size="small" />}
                        label={<Box><Typography variant="body2" fontWeight={600}>Internal ID Range</Typography><Typography variant="caption" color="text.secondary">Enter start and end bounds</Typography></Box>}
                ...
                <Box sx={{ maxWidth: 600, mx: "auto" }}>
                  {batchMethod === "csv" ? (
                    ...
                    >
                      ...
                      <Typography variant="caption" color="text.disabled">{csvFile ? `${(csvFile.size / 1024).toFixed(1)} KB` : "Accepts .csv files • Reseller IDs in column A"}</Typography>
                      <input ref={fileRef} type="file" accept=".csv" hidden onChange={(e) => setCsvFile(e.target.files?.[0] || null)} />
                    </Box>
                  ) : (
                    <Stack spacing={2}>
                      <Typography variant="body2" fontWeight={600}>Enter ID range:</Typography>
              ...
              {activeStep === 2 && (
                <Box sx={{ maxWidth: 600, mx: "auto", py: 4, textAlign: "center" }}>
                  <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>Validating reseller IDs against system records...</Typography>
              ...
              {activeStep === 3 && (
                <Box sx={{ maxWidth: 800, mx: "auto" }}>
                      ...
                      <TableBody>
                        {MOCK_BATCH_RESULTS.map((row, i) => (
                          <TableRow key={i} hover>
                            <TableCell><Typography variant="body2" sx={{ fontFamily: "monospace", fontSize: "0.8rem", fontWeight: 700, color: "primary.main" }}>{row.id}</Typography></TableCell>
                  ...
                  <Stack direction="row" spacing={1} sx={{ mt: 3, justifyContent: "flex-end" }}>
                    ...
                    <Button component={NextLink} href="/resellers" variant="contained" size="small" sx={{ bgcolor: "var(--safaricom-green)", fontWeight: 600 }}>Done</Button>
                  </Stack>
                </Box>
              )}
           </Box>
